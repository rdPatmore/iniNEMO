...
def plot_N_M_histogram():
    ...
    N2 = xr.open_dataset(path + "bn2_ml_mid_ice_oce_miz_model_hist.nc")
    M2 = xr.open_dataset(path + "bg_mod2_ml_mid_ice_oce_miz_model_hist.nc")
    M2N2 = xr.open_dataset(
                      path + "M2_over_N2_ml_mid_ice_oce_miz_model_hist.nc")

    # initailise plot
    fig, axs = plt.subplots(3,figsize=(5.5,6.5))
    plt.subplots_adjust(hspace=0.5)

    def render(N2, M2, M2N2, partition, c='r'):
        N2 = N2['hist_bn2_ml_mid_' + partition]
        M2 = M2['hist_bg_mod2_ml_mid_' + partition]
        M2N2 = M2N2['hist_M2_over_N2_ml_mid_' + partition]
    ...
    # plot
    render(N2, M2, M2N2, 'ice', c='r')
    render(N2, M2, M2N2, 'oce', c='g')
    render(N2, M2, M2N2, 'miz', c='b')
...
def plot_N_M_histogram_2d():
    ...
    M2N2_2d = xr.open_dataset(
             path + "bg_mod2_bn2_ml_mid_ice_oce_miz_model_hist.nc")

    # initailise plot
    fig, axs = plt.subplots(3 ,figsize=(3.2,5.5))
    plt.subplots_adjust(right=0.80, top=0.98)

    def render(ax, partition):
        p = ax.pcolor(M2N2_2d.x_bin_centers, M2N2_2d.y_bin_centers,
                       M2N2_2d['hist_bg_mod2_ml_mid_' + partition + 
                               '_bn2_ml_mid_' + partition].T,
                       norm=mc.LogNorm())
