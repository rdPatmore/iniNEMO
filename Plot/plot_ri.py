...
def plot_ri_proportion():
    def ri_proportion(case, ax, depth):
        ...
        d_unstab = xr.where((0<=ri) & (ri<0.25), area, 0).sum(['x','y']) /\
                   area_sum
        m_stab = xr.where((0.25<=ri) & (ri<1), area, 0).sum(['x','y']) /\
                    area_sum
        ...
        ax.fill_between(seg1.time_counter, seg0, seg1, facecolor='royalblue',
                                label='0.25<=Ri<1')
        ax.fill_between(seg2.time_counter, seg1, seg2,
...
def plot_ri_10m():
    ''' plot two-dimensional slice of Ri '''
 
    # initialise
    fig, axs = plt.subplots(3,3, figsize=(5.5,5.5))
    plt.subplots_adjust()
    ...
    ri = xr.open_dataarray(config.data_path() + '/' + case + 
                          '/richardson_number.nc',
                          chunks=dict(deptht=1))
    print (ri)

    # define slices
    times = ['2012-12-24 12:00:00',
             '2013-01-08 12:00:00',
             '2013-02-08 12:00:00']
    depths = [1, 10, 50]
    ...
    for j, depth in enumerate(depths):
        for i, time in enumerate(times):
            print (i, ' ', j)
            ri_slice = ri.sel(time_counter=time, deptht=depth,
                        method='nearest').load()
            axs[i,j].pcolor(ri_slice, cmap=cmocean.cm.balance, vmin=-1, vmax=1)

    plt.savefig(case + '_ri_2d_depth_time_slices.png')
